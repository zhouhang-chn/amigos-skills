...
| `skills/` | The `/amigos` skill. |
| `integrations/` | Host adapters over the gate: Claude Code, git. |
| `.claude-plugin/` | Claude Code plugin manifest. |

`skills/` ships the `/amigos` skill and is declared in the plugin manifest.
`agents/` arrives in v0.4 when the three perspectives become independent
subagents.
...
Milestones v0.1 through v0.3 are implemented: the contract format, the
deterministic validator, the acceptance linter, story scaffolding, the
repository gate with its Claude Code and git adapters, and the `/amigos` skill.

Contracts are no longer hand-authored by default. STORY-005's contract was
generated by the skill and passes the same validator every hand-authored
contract passes.
